...
Percent.removeEventListener("click", btnClick);
Percent.addEventListener("click", calculatePercent);

// פונקציה לחישוב תוצאה (ללא eval - חישוב מאובטח)
function safeEval(expr) {
    const cleaned = String(expr).replace(/[^\d+\-*/().%\s]/g, "");
...
function calculate() {
    ...
    try {
        const result = safeEval(valueEl.textContent);
...
function calculatePercent() {
    ...
    if (!isNaN(currentValue)) {
        ...
        console.log("תוצאת האחוז:", valueEl.textContent);
    } else {
        valueEl.textContent = "Error";
